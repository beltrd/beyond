<?php
    ...
    $pageNumber = empty($params[1]) ? 1 : $params[1];
    $itemsPerPage = empty($params[2]) ? $pagination[0] : $params[2];
    ...
    // get category
    $category = \Models\Category::getById($params[0]);
                            ...
                            <div class="toolbar">
                                ...
                                <div class="pager">
                                    <div class="limiter">
                                        ...
                                        <ul>
                                            <li>
                                                <a href="/categories/list/<?=$category['id']?>/1/<?=$itemsPerPage?>"><?=$itemsPerPage?><span class="right-arrow"></span></a>
                                                <ul>
<?php
...
?>
                                                    <li><a href="/categories/list/<?=$category['id']?>/1/<?=$value?>"><?=$value?></a></li>
<?php } //foreach ?>
                                                </ul>
                                            </li>
                                        </ul>
                                    ...
                                    <div class="pages">
                                        ...
                                        <label>Page:</label>
                                        <ul class="pagination">
                                            <li><a href="/categories/list/<?=$category['id']?>/<?= max(1,$pageNumber-1) ?>/<?=$itemsPerPage?>">&laquo;</a></li>
<?php
    ...
    for ($i=1; $i<=$pagesNum; $i++) {
        ...
        if ($pagesNum > 5) {
            if ($pageNumber==$i
                ...
                // show page marker
?>
                                            <li<?= $pageNumber==$i ? ' class="active"' : '' ?>><a<?= $pageNumber==$i ? ' class="a-disabled"' : '' ?> href="/categories/list/<?=$category['id']?>/<?=$i?>/<?=$itemsPerPage?>"><?=$i?></a></li>
<?php
                unset($dividerShown);
                continue;
            } else {
                if (empty($dividerShown)) {
                    //show divider
?>
                                            <li><a class="a-disabled" href="/categories/list/<?=$category['id']?>/<?=$pageNumber?>/<?=$itemsPerPage?>">&hellip;</a></li>
<?php
...
<?php }} //if //for ?>
                                            <li><a href="/categories/list/<?=$category['id']?>/<?= min($pagesNum,$pageNumber+1) ?>/<?=$itemsPerPage?>">&raquo;</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
